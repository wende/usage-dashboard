...
import { formatAsof, formatReset, windowMeta } from "../time.js";
import { StaleError } from "../errors.js";
import { pct, request } from "./common.js";
...
export async function fetchGrok() {
  ...
  const res = await request("grok", USAGE_URL, {
    method: "POST",
    headers: {
      accept: "*/*",
  ...
  return {
    asof: formatAsof(),
    weekly: {
      pct: pct(config.creditUsagePercent),
      reset: formatReset(resetDate),
      ...windowMeta(resetDate, {
        startDate: config.currentPeriod?.start,
        period: config.currentPeriod?.type === "monthly" ? "monthly" : "weekly",
      }),
    },
    plan: cred.plan || "",
